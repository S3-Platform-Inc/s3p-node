...
class SaveDocumentToDB(BaseModule):
    ...
    def _save(self, doc: SPP_document) -> SPP_document:
        ...
        if new_doc.id == doc.id:
            self.logger.info(f'Update document title:{doc.title}, weblink: {doc.web_link}, pubdate:{doc.pub_date} to database')
        else:
            self.logger.info(f'Save document title:{doc.title}, weblink: {doc.web_link}, pubdate:{doc.pub_date} to database')
        return new_doc
