...
class SppFileServerBroker:
    ...
    def upload_document(self, document: S3PDocument, bin_doc: bytes | io.BytesIO | BinaryIO) -> bool | str:
        """
    ...
    def document(self, document: S3PDocument) -> io.BytesIO:
        """
    ...
    def _safe_download(self, sourced_session: FTP, doc: S3PDocument) -> io.BytesIO:
        try:
            _bytes_stream = io.BytesIO()
            if doc.storage:
                sourced_session.retrbinary(f"RETR {doc.storage}", _bytes_stream.write)
    ...
    def __safe_upload(self, session: FTP, doc: S3PDocument, binaryfile: io.BytesIO | BinaryIO) -> tuple[str, FTP]:
        try:
            int_pad = 0
            pad = ''
    ...
    def __upload(self, session: FTP, doc: S3PDocument, binaryfile: io.BytesIO) -> FTP:
        try:
    ...
    def __document_filename(self, doc: S3PDocument, pad: str = "") -> str:
        concat_name = doc.title + '_' + doc.link + '_' + str(doc.published.timestamp())

        return str(
            sha224(concat_name.encode('utf8')).hexdigest()) + f"_{doc.title}_{str(doc.published.timestamp())}_({pad})"
...
if __name__ == "__main__":
    fs = SppFileServerBroker(S3PRefer(3, 'r3r', '', datetime.datetime.now()))
    ...
    fs.upload_document(S3PDocument(1, '1', '2', '3', '4', '5', {}, dt, dt), d)
